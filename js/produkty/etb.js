document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        // pole, jen etb
        const products = Object.entries(data).filter(([key, product]) => product.type === 'etb');

        if (products.length === 0) {
            container.innerHTML = '<p class="text-center">Momentálně nejsou k dispozici žádné Elite Trainer Boxy.</p>';
            return;
        }
